...
func TestSetEdge(t *testing.T) {
	n, err := tDq.Dgraph.NodeBlank("test_set_edge")
	if err != nil {
		t.Error(err)
		return
	}
	...
	e := n.Edge(edgeName)
	err = tDq.setEdge(&e, reflect.ValueOf(0))
	if err != nil {
		t.Error(err)
		return
	}
	err = tDq.setEdge(&e, reflect.ValueOf(true))
	if err != nil {
		t.Error(err)
		return
	}
	err = tDq.setEdge(&e, reflect.ValueOf(map[string]string{"waifu": "you"}))
	if err != nil {
		t.Error(err)
		return
	}
	err = tDq.setEdge(&e, reflect.ValueOf("testscalar"))
	if err != nil {
		t.Error(err)
		return
	}
	...
	_, err = tDq.Dgraph.Run(context.Background(), req)
	if err != nil {
		t.Error(err)
		return
	}
...
func TestSetEdgeErr_Quotes(t *testing.T) {
	n, err := tDq.Dgraph.NodeBlank("test_set_edge_err_quotes")
	if err != nil {
		t.Error(err)
		return
	}
	...
	edgeName := fmt.Sprintf("testedge_%d", rand.Int31())
	e := n.Edge(edgeName)
	err = tDq.setEdge(&e, reflect.ValueOf(`"this has quotes!"`))
	if err != nil {
		t.Error(err)
		return
	}
